      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-6">
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-2.5 sm:p-6 flex items-center gap-2.5 sm:gap-5 shadow-xs hover:shadow-md transition-all duration-200 col-span-1">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[8px] sm:text-xs font-black text-zinc-500 uppercase tracking-wider truncate">Total Bazaar</p>
            <h3 className="text-xs sm:text-2xl font-black text-zinc-900 mt-0.5 sm:mt-1 truncate">৳{totalBazaar.toFixed(0)}</h3>
            <p className="text-[10px] text-zinc-400 mt-0.5 font-medium truncate hidden sm:block">Member deposits</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-2.5 sm:p-6 flex items-center gap-2.5 sm:gap-5 shadow-xs hover:shadow-md transition-all duration-200 col-span-1">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[8px] sm:text-xs font-black text-zinc-500 uppercase tracking-wider truncate">Total Meals</p>
            <h3 className="text-xs sm:text-2xl font-black text-zinc-900 mt-0.5 sm:mt-1 truncate">{totalMeals.toFixed(1)}</h3>
            <p className="text-[10px] text-zinc-400 mt-0.5 font-medium truncate hidden sm:block">Meals eaten</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-zinc-200 rounded-2xl p-2.5 sm:p-6 flex items-center gap-2.5 sm:gap-5 shadow-xs hover:shadow-md transition-all duration-200 relative overflow-hidden col-span-2 sm:col-span-1">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[8px] sm:text-xs font-black text-zinc-500 uppercase tracking-wider truncate">Meal Rate</p>
            <h3 className="text-xs sm:text-2xl font-black text-zinc-900 mt-0.5 sm:mt-1 truncate">
              ৳{currentMealRate.toFixed(2)}
            </h3>
            <p className="text-[10px] text-zinc-400 mt-0.5 font-medium truncate hidden sm:block">Bazaar / Total Meals</p>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        {results.length === 0 ? (
          <div className="text-center py-12 bg-white border border-zinc-200 rounded-2xl text-zinc-450 text-sm font-semibold shadow-3xs">
            ...
            <p className="text-zinc-600 text-xs font-extrabold">No Members Yet</p>
            <p className="text-[10px] text-zinc-400 mt-1">Click Add Member above to register bachelor members.</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div>
                        ...
                        <div className="mt-1 flex flex-wrap gap-1">
                          {isOwed && (
                            <span className="flex items-center gap-0.5 text-[8px] font-extrabold bg-emerald-50 text-emerald-800 px-1.5 py-0.5 rounded-md uppercase tracking-wider">
                              <TrendingUp className="w-2.5 h-2.5" /> Receives
                            </span>
                          )}
                          {isOwes && (
                            <span className="flex items-center gap-0.5 text-[8px] font-extrabold bg-rose-50 text-rose-800 px-1.5 py-0.5 rounded-md uppercase tracking-wider">
                              <TrendingDown className="w-2.5 h-2.5" /> Owes
                            </span>
                          )}
                          {!isOwed && !isOwes && (
                    ...
                    <div className="mt-4 space-y-1.5 border-t border-zinc-100 pt-3 text-[10px] sm:text-xs">
                      <div className="flex justify-between items-center text-zinc-500">
                        <span className="hidden sm:inline">Bazaar Deposit:</span>
                        <span className="inline sm:hidden">Deposit:</span>
                        <span className="font-bold text-zinc-700">৳{(member.bazaarAmount || 0).toFixed(1)}</span>
                      </div>
                      <div className="flex justify-between items-center text-zinc-500">
                        <span className="hidden sm:inline">Meals ({member.totalMeals || 0}):</span>
                        <span className="inline sm:hidden">Meals ({member.totalMeals || 0}):</span>
                        <span className="font-bold text-zinc-700">৳{mealExpense.toFixed(1)}</span>
                      </div>
                      <div className="flex justify-between items-center text-zinc-500">
                        <span className="hidden sm:inline">Adjustments:</span>
                        <span className="inline sm:hidden">Adjusts:</span>
                    ...
                    {/* Present list of individual custom costs inside the sheet card so member ledger is not needed */}
                  ...
                  {/* Summary Box Footer */}
                  <div className="mt-4 pt-2.5 border-t border-zinc-100 bg-zinc-50/50 -mx-5 -mb-5 px-5 py-2.5 rounded-b-2xl flex justify-between items-center">
                    <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider hidden sm:inline">Total Personal Cost</span>
                    <span className="text-[9px] font-extrabold text-zinc-400 uppercase tracking-wider inline sm:hidden">Total Cost</span>
